...
# everything derives from $HOME (or $HERMES_HOME) and the flags you pass.
#
# Usage:
...
#   Install location overrides: HERMES_HOME, HERMES_PY, HERMES_BIN.
#
# Rollback / removal: rm-bot.sh <name>
set -euo pipefail

# Resolve our own directory ONCE, before any cd: a later relative "$(dirname $0)"
# lookup breaks the moment the working directory changes.
...
HV="${HERMES_PY:-$MAIN/hermes-agent/venv/bin/python}"
HERMES="${HERMES_BIN:-$MAIN/hermes-agent/venv/bin/hermes}"
PLAT_UC="$(echo "$PLATFORM" | tr '[:lower:]' '[:upper:]')"
OTHER_UC=$([ "$PLAT_UC" = BALE ] && echo SOROUSH || echo BALE)

[[ -x "$HV" ]] || { echo "ERROR: no hermes venv python at $HV (set HERMES_PY if your install differs)" >&2; exit 1; }
[[ -x "$HERMES" ]] || { echo "ERROR: no hermes CLI at $HERMES (set HERMES_BIN if your install differs)" >&2; exit 1; }
...
if [ -f "$MAIN/.env" ]; then cp "$MAIN/.env" "$PROF/.env"; else : > "$PROF/.env"; echo "   (no $MAIN/.env yet — starting from an empty one)"; fi
"$HV" - "$PROF/.env" "$PLAT_UC" "$OTHER_UC" "$TOKEN" "$USER_ID" "$ADMINS" "$HOME_CHAT" <<'PY'
import sys
path, plat, other, token, user, admins, home = sys.argv[1:8]
users = [x for x in [user] + (admins.split(",") if admins else []) if x]
chats = list(users) + ([home] if home and home not in users else [])
allowed_users, allowed_chats = ",".join(users), ",".join(chats)
lines = open(path, encoding="utf-8", errors="surrogateescape").read().split("\n")
out, seen = [], set()
kept = 0
for ln in lines:
    key = ln.split("=", 1)[0].strip() if "=" in ln else ""
    # 1) nothing that belongs to the OTHER platform travels into this profile:
    #    its token would start a second poller, its allowlists/home/chat ids are
    #    the host's private data.
    if key.startswith(other + "_"):
        out.append(f"#{ln}"); continue
    if key == f"{plat}_BOT_TOKEN":
        out.append(f"{plat}_BOT_TOKEN={token}"); seen.add(key); continue
    if key == f"{plat}_ALLOWED_USERS":
        out.append(f"{key}={allowed_users}"); seen.add(key); continue
    if key == f"{plat}_ALLOWED_CHATS":
        out.append(f"{key}={allowed_chats}"); seen.add(key); continue
    ...
    if key == f"{plat}_CHAT_ID":
        # the main .env's own chat id is NOT this bot's chat
        out.append(f"{plat}_CHAT_ID={home or user}"); seen.add(key); continue
    if key == "GATEWAY_ALLOW_ALL_USERS":
        out.append("GATEWAY_ALLOW_ALL_USERS=false"); seen.add(key); continue
    # 2) host-only admin secrets. Shared LLM keys are deliberately KEPT: every
    #    profile runs on the host's model credentials (see SKILL.md).
    if key in {"SUDO_PASSWORD", "BROWSERBASE_API_KEY", "BROWSERBASE_PROJECT_ID",
        ...
        out.append(f"#{ln}"); continue
    out.append(ln)
for key, val in ((f"{plat}_BOT_TOKEN", token), (f"{plat}_ALLOWED_USERS", allowed_users),
                 (f"{plat}_ALLOWED_CHATS", allowed_chats), (f"{plat}_ALLOW_ALL_USERS", "false"),
                 (f"{plat}_HOME_CHANNEL", home or user), (f"{plat}_CHAT_ID", home or user),
...
# Assert the promises above instead of trusting the writer.
if grep -qE '^SUDO_PASSWORD=' "$PROF/.env"; then echo "ERROR: SUDO_PASSWORD leaked into $PROF/.env" >&2; exit 1; fi
if grep -qE "^${OTHER_UC}_" "$PROF/.env"; then
  echo "ERROR: ${OTHER_UC} values are active in $PROF/.env (host data / double-poller risk):" >&2
  grep -E "^${OTHER_UC}_" "$PROF/.env" | cut -d= -f1 | sed 's/^/       /' >&2; exit 1
fi
if [ "$(grep -cE '^[A-Z0-9]+_BOT_TOKEN=' "$PROF/.env")" -gt 1 ]; then
  echo "ERROR: more than one active *_BOT_TOKEN in $PROF/.env" >&2; exit 1
fi
echo "   .env: ${PLAT_UC}_* set, ${OTHER_UC}_* neutralised, AllowAll=false, secrets stripped"
...
# fail fast on the dependency the adapters need, instead of debugging a silent
# 'Plugin discovery complete' with no 'Connected as' later
if ! "$HV" -c "import aiohttp" >/dev/null 2>&1; then
  echo "   WARNING: aiohttp is not importable in $HV — the adapters need it: $HV -m pip install aiohttp" >&2
fi

echo "==> 4/6 config.yaml"
"$HV" - "$NAME" "$PLATFORM" "$PROF" "$H" "$MAIN" "$SANDBOX_IMAGE" "$USER_ID" "$HOME_CHAT" <<'PY'
import os, sys, yaml
name, plat, prof, h, main, sandbox_image, user_id, home_chat = sys.argv[1:9]
main_cfg = os.path.join(main, "config.yaml")
cfg = yaml.safe_load(open(main_cfg, encoding="utf-8")) if os.path.isfile(main_cfg) else {}
other = "soroush" if plat == "bale" else "bale"
chat = str(home_chat or user_id)
cfg["platforms"] = {
    "api_server": {"enabled": False},
    plat: {"enabled": True, "home_channel": {"platform": plat, "chat_id": chat, "user_id": str(user_id)}},
    other: {"enabled": False},
}
...
# NOTE: the whole plugins block is replaced, so inherited plugin settings
# (e.g. a host-level memory-store path) do NOT carry over; the memory store falls
# back to its per-profile default. Set it explicitly here if you need a shared one.
cfg["plugins"] = {"disabled": [], "enabled": [f"platforms/{plat}"],
                  "entries": {f"platforms/{plat}": {"allow_tool_override": False}},
                  "hermes-memory-store": {"db_path": os.path.join(main, "profiles", name, "memory_store.db")}}
...
if sandbox_image:
    term.update({
        ...
        # mount the profile's OWN skills (never the shared main tree) so one bot
        # cannot read another profile's material
        "docker_volumes": [f"{h}/workspaces/{name}:{h}/workspaces/{name}",
                           f"{main}/profiles/{name}:{main}/profiles/{name}:ro",
                           f"{main}/profiles/{name}/skills:{main}/skills:ro"],
    })
else:
    term.update({"backend": "local"})
yaml.safe_dump(cfg, open(os.path.join(prof, "config.yaml"), "w"), allow_unicode=True, sort_keys=False)
print(f"   wrote config.yaml (platform={plat}, home_chat={chat}, terminal.backend={term.get('backend')})")
PY
