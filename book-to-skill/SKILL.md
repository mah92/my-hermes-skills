...
metadata:
  hermes:
    related_skills: [hermes-libgen-book-download-skill]
---
...
**Need the source file first?** If the user names a book or document but no local file exists, fetch it with the related **hermes-libgen-book-download-skill** (working mirror from this network: libgen.li — see that skill for the verified download flow), then run this converter on the downloaded file.
...
0. **Hermes personal skills: `~/.hermes/skills/`** — this bot runs Hermes Agent; generated book skills must land here (that is the directory Hermes scans for skills). Also update/check the existing skill folder under `~/.hermes/skills/<slug>/` for Update/Fold-in runs.
...
SCRIPT_PATH=""
for candidate in \
  "$HOME/.hermes/skills/book-to-skill/scripts/extract.py" \
...
|---|---|---|
| **Hermes Agent** | `~/.hermes/skills` (the only root this host scans) | — |
